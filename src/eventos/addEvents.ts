...
export namespace addEventsHandler {
    ...
    async function InserirEvento(titulo: string, desc: string, data_inicio: string, data_hora_inicio_apostas: string, data_hora_fim_apostas: string, categoria:string, token: string): Promise<boolean | undefined> {
        ...
        try {
            ...
            await conn.execute(
                `INSERT INTO evento (id_evento, titulo, descricao, data_inicio, data_hora_inicio_apostas, data_hora_fim_apostas, id_usuario_fk, status, categoria) 
                VALUES (seq_id_eventos.NEXTVAL, :titulo, :descricao, TO_TIMESTAMP(:data_inicio, 'DD/MM/YYYY HH24:MI:SS'), 
                TO_TIMESTAMP(:data_hora_inicio_apostas, 'DD/MM/YYYY HH24:MI:SS'), TO_TIMESTAMP(:data_hora_fim_apostas, 'DD/MM/YYYY HH24:MI:SS'), :idUser, 'aguarda aprovação', :categoria)`,
                {
                    titulo: titulo,
                    descricao: desc,  
    ...
    export const addNewEventHandler: RequestHandler = async (req: Request, res: Response) => {
        ...
        const { titulo, desc, dataInicio, dataInicioApostas, horaInicioApostas, dataFimApostas, horaFimApostas, categoria } = req.body;
